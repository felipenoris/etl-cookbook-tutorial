...
rm -rf examples-DuckDB/examples/_tmp_spill examples-rust-extension/_tmp_spill
echo "ok"
...
if $DEEP; then
    ...
    for proj in examples-pandas examples-pyarrow examples-DuckDB examples-rust-extension examples-sqlalchemy-contract; do
        rm -rf "$proj/.venv" "$proj/uv.lock"
    done
    rm -f examples-rust-extension/Cargo.lock
