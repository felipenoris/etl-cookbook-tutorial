...
1. **DuckDB** (`../examples-DuckDB`) lê e junta `orders` + `customers` + `products` de
...
2. **pyarrow** (`../examples-pyarrow`) faz uma pequena projeção/cast antes de repassar
...
4. **pandas** (`../examples-pandas`) com backend Arrow resume o resultado final por
...
def extract_and_join_with_duckdb() -> pa.Table:
    ...
    ``examples-DuckDB/examples/04_memory_limit_and_spill.py``, para mostrar que o join
...
def summarize_with_pandas(table: pa.Table) -> pd.DataFrame:
    """Resumo final com pandas usando backend Arrow (mesmo padrão de ``../examples-pandas``)."""
